...
/**
 * 
 * <pre><b>a tool to detect which type the sql .</b></pre>
 * @author 
 * <pre>seaboat</pre>
...
public final class SqlTypeParser {
	...
	public static final int TRUNCATE = 26;
	...
	public static int parse(String stmt) {
		...
		for (int i = 0; i < lenth; ++i) {
			...
			case '/':
				...
				// */;
				if (i == 0 && stmt.charAt(1) == '*') {
					switch (stmt.charAt(2)) {
					case '!':
						if (checkCommentTail(stmt, lenth)) {
							return MYSQL_CMD_COMMENT;
						}
						break;
					case ' ':
						if (stmt.charAt(3) == '!'
								&& checkCommentTail(stmt, lenth)) {
							continue;
						}
						break;
					}
	...
	private static boolean checkCommentTail(String stmt, int length) {
		return stmt.charAt(length - 2) == '*' && stmt.charAt(length - 1) == '/';
	}
	...
	static int commitCheck(String stmt, int offset) {
		if (stmt.length() > offset + 5) {
			...
			char c4 = stmt.charAt(++offset);
			char c5 = stmt.charAt(++offset);
			if ((c1 == 'O' || c1 == 'o') && (c2 == 'M' || c2 == 'm')
					&& (c3 == 'M' || c3 == 'm') && (c4 == 'I' || c4 == 'i')
					&& (c5 == 'T' || c5 == 't') && (stmt.length() == ++offset)) {
				return COMMIT;
			}
		}
	...
	// DESCRIBE or desc or DELETE' ' or DROP' '
	static int dCheck(String stmt, int offset) {
		if (stmt.length() > offset + 3) {
			int res = dropCheck(stmt, offset);
			if (res == DROP) {
				return res;
			}
		}
	...
	static int rCheck(String stmt, int offset) {
		if (stmt.length() > ++offset) {
			...
			case 'O':
			case 'o':
				int checkPoint = rollabckCheck(stmt, offset);
				return ROLLBACK == checkPoint ? checkPoint : routeCheck(stmt,
						offset);
			default:
				return OTHER;
			}
	...
	// ROUTE
	static int routeCheck(String stmt, int offset) {
		if (stmt.length() > offset + 6) {
			char c1 = stmt.charAt(++offset);
			char c2 = stmt.charAt(++offset);
			char c3 = stmt.charAt(++offset);
	...
	static int tCheck(String stmt, int offset) {
		if (stmt.length() > ++offset) {
			switch (stmt.charAt(offset)) {
			case 'R':
			case 'r':
				return truncateCheck(stmt, offset);
			default:
				return OTHER;
			}
		}
	...
	// TRUNCATE' '
	static int truncateCheck(String stmt, int offset) {
		if (stmt.length() > offset + 8) {
			char c1 = stmt.charAt(++offset);
			char c2 = stmt.charAt(++offset);
			char c3 = stmt.charAt(++offset);
